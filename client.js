...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AgentBrowserView(props) {
      ...
      /** The definite-size frame box; observing THIS (not the img) avoids the size feedback lock. */
      ...
      // Keep the page the size of the panel's content box. Observed on the BOX,
      // not the img: the img is sized by the frame, so feeding its size back to
      // the page would lock the panel at whatever size it first had.
      react.useEffect(function () {
      ...
      // The frame box is a definite, absolutely-filled area. An <img> left to
      // size itself sits at the FRAME's intrinsic width, and because that width
      // is what the viewport sync feeds back to the page, the panel then locks
      // at its first size and never grows when the sidebar is widened.
      var box = h(
        'div',
        {
          ref: boxRef,
